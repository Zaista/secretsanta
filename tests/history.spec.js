...
test.describe('history tests', () => {
  test('user can edit santa history', async ({ page }) => {
    ...
    await expect(page.getByText('Year location was updated successfully')).toBeVisible();
    ...
    await page.getByText('Save').click();
    await expect(page.getByText('Gift description was updated successfully')).toBeVisible();
    ...
    await expect(page.getByText('Location image was uploaded successfully')).toBeVisible();
    await expect(page.locator('#locationImage')).toHaveAttribute('src', /data:image\/png/);

    await page.locator('#giftIcon').nth(0).click();
    await page.getByText('Change image').click();
    await page.locator('#imageUpload').setInputFiles('tests/santaGift.jpg');
    await page.getByRole('button', { name: 'Upload' }).click();
    await expect(page.getByText('Gift image was uploaded successfully')).toBeVisible();
    await expect(page.locator('#giftImage').nth(0)).toHaveAttribute('src', /data:image\/png/);

    await page.getByText('History').click();
    await expect(page.locator('#locationImage')).toHaveAttribute('src', /history\/year\/api\/location-image\?id=/);
